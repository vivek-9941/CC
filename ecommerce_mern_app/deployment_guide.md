# MERN E-Commerce App - VM Deployment Guide

This guide details exactly how to deploy your "Proper MERN" E-Commerce Application on a fresh Cloud Virtual Machine.
...
```bash
scp -i /path/to/your-key.key -r "d:\insem\LP2\ecommerce_mern_app" ubuntu@<YOUR_VM_PUBLIC_IP>:~/
```
...
```bash
cd ~/ecommerce_mern_app/backend
pm2 start server.js --name "ecommerce-api"
pm2 save
```
Browse to `http://<YOUR_VM_PUBLIC_IP>` to see the shop! Default products are automatically seeded on first launch.
...
sudo nano /etc/nginx/sites-available/mern_ecommerce
```
Paste the following, modifying `/home/ubuntu/ecommerce_mern_app` if your path is different:
```nginx
server {
    listen 80;
    server_name _;
    
    root /home/ubuntu/ecommerce_mern_app/frontend/dist;
    index index.html;
...
Activate it:
```bash
sudo ln -s /etc/nginx/sites-available/mern_ecommerce /etc/nginx/sites-enabled/
